...
ScopeDomeArduino::ScopeDomeArduino(bool useEthernet, int fd) : ethernet(useEthernet), PortFD(fd)
{
    ...
    nameMap["INPD_HOMESENSOR"] = "Home sensor";
    //    nameMap[""]
}
...
bool ScopeDomeArduino::performCommand(const std::string &command, std::string &response)
{
    if (ethernet)
    {
        ...
        snprintf(requestURL, MAXRBUF, "http://192.168.1.100/?%s", command.c_str());
    ...
    else
    {
        ...
        char lineBuf[MAXRBUF];
        snprintf(lineBuf, MAXRBUF - 1, "%s\r\n", command.c_str());
...
{
    return (rotaryEncoder - 32000) % stepsPerRevolution;
}

int ScopeDomeArduino::getRotationCounterExt()
{
    return rotaryEncoder - 32000;
}
...
{
    std::string res;
    performCommand("resetEncoderAPositionCounter", res);
}
...
void ScopeDomeArduino::move(int steps)
{
    ...
    {
        char cmd[32] = {0};
        snprintf(cmd, 32, "moveDome=CCW:%d", -steps);
        performCommand(cmd, res);
    }
    else
    {
        char cmd[32] = {0};
        snprintf(cmd, 32, "moveDome=CW:%d", steps);
        performCommand(cmd, res);
    }
}
